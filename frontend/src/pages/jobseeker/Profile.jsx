...
const Profile = () => {
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      // Profile image - fetch and convert to base64
      const imgUrl = profilePicture || user?.profilePicture;
      const imgBase64 = await getImageBase64(imgUrl);
      if (imgBase64) {
        try {
          doc.addImage(imgBase64, 'JPEG', 10, 8, 48, 48);
        } catch (e) { /* skip */ }
      }
      ...
      if (pdfSkills && pdfSkills.length > 0) {
        ...
        doc.setTextColor(67, 56, 202);
        doc.setFontSize(14);
        doc.text('TECHNICAL SKILLS', 14, yPos + 6);
        yPos += 15;
