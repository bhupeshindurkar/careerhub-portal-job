...
// @desc    Seed real jobs (Admin only)
// @route   POST /api/seed/real-jobs
// @access  Private/Admin
router.post('/real-jobs', protect, authorize('admin'), async (req, res) => {
  try {
    const realJobs = [
      {
    ...
    // Create or find employer accounts
    ...
    res.json({
      status: 'success',
      message: `Successfully seeded ${insertedJobs.length} real jobs from ${companies.length} companies`,
      data: {
